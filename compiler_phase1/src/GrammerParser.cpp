...
Graph* GrammerParser::getFullNFA()
{
    ...
    for(int i=0; i< expNames.size(); i++) //lambda from start for every expression
    {
...
void GrammerParser::getNFA(string line)
{
    ...
    if(regex_match(line,splitter1))
    {
        ...
        string str = match1[1];//after {
        //cout << str << endl;
        while(regex_search(str,match1,spaceSplitter))
        {
            string tok = match1[0];//first token
            ...
            for(int i = 0; i < token.length(); i++)//making state for every character in tokens
            {
                string c = "";
                c.push_back(token[i]);
                //cout << c << endl;
            ...
            //       setWeights(g);
            str = match1.suffix().str();//whole sting after this token
        }
    }
    ...
    else if (regex_match(line,splitter3))
    {
        ...
        string name = match1[1];//letter
        //cout<<name<<endl;
        string str2 = match1[3];//a-z | A-Z
        //cout<<str2<<endl;
        vector<string> tokens = getTokens(str2);//a-z ... | ... A-Z
        stack<string> postfix = getPostfix(tokens);//how the operations will done on tokens ==> a-z  A-Z |
...
{
    string str = "";
    for(int i=0; i<str2.length(); i++)//for putting spaces between anything will be a state
    {
        char c = str2[i];
        if(isOperation(c))
        {
    ...
    while(regex_search(str,match1,spaceSplitter))
    {
        ...
        if(token.length() == 2 && token[0] == '\\' && token[1] == 'L')//useless
        {
            token = "";
        }
...
stack<string> GrammerParser::getPostfix(vector<string> tokens)//how the operations will done on tokens
{
    stack<string> post;
    stack<string> in;
    string cha,ch;
    for(int j=0; j < tokens.size(); j++)
    {
        ...
        {
            if(!token.compare("("))//token.compare("("))==0
            {
                if (j != 0 && tokens[j - 1].compare("|")){
                    addOperation('#',&in,&post);//and operation
                }
                in.push(token);
            }
...
Graph* GrammerParser::draw(stack<string> postfix, string acceptance)
{
    ...
    while(postfix.size()!=0)
    {
        ...
        {
            Graph *g = new Graph();
            regex split1("(\\S+)-(\\S+)");//range like a-z
            std::smatch match1;
            if(regex_match(token,split1))//all characters from a to z ...
            {
            ...
            else
            {
                ...
                if(found != definitions.end())//letter or digit
                {

                    g = clone(definitions[token]);//return new graph with same state numbers
                }
...
Graph* GrammerParser::clone(Graph* g)//return new graph with same state numbers
{
    ...
    while(!current.empty())
    {
        ...
        if(tmp->get_state_num() == oldnum)//start of graph
        {
